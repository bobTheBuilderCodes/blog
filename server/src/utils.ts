...
export async function requireAdmin(req:Request,res:Response,next:NextFunction){if(env.demoMode)return next();try{const token=req.signedCookies?.margin_session;if(!token)return res.status(401).json({message:'Sign in is required.'});const session=await AdminSession.findOne({tokenHash:hash(token),expiresAt:{$gt:new Date()}});if(!session)return res.status(401).json({message:'Your session has expired.'});next()}catch{res.status(401).json({message:'Sign in is required.'})}}
export const cookieOptions={httpOnly:true,sameSite:(env.production?'none':'lax') as 'none'|'lax',secure:env.production,signed:true,path:'/',maxAge:1000*60*60*24*14}
