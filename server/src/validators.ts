...
export const articleSchema=z.object({title:z.string().trim().min(3).max(180),slug:z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/),excerpt:z.string().trim().min(10).max(500),content:z.string().min(3),coverImage:cover,category:z.string().trim().min(2).max(50),tags:z.array(z.string().trim().max(40)).max(12).default([]),author:z.object({name:z.string().trim().min(2).max(80),avatar:z.string().url().optional().or(z.literal(''))}),status:z.enum(['draft','published']).default('draft'),featured:z.boolean().default(false)}).strict()
